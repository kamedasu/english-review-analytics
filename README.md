...
`OPENAI_API_KEY` が未設定、またはAPI呼び出しに失敗した場合、アプリはrule-based summaryに自動でフォールバックします。画面の `Monthly Summary` に `Generated by OpenAI API` または `Generated by rule-based fallback` と表示されます。
