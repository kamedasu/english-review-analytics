...
5. Notion未設定でもサンプルデータで動作確認できるようにする
...
NotionページIDは、月別ページをカンマ区切りで指定します。未設定の場合は `data/raw/sample_reviews.md` を使って表示します。
...
サイドバーの `Use sample data` を有効にすると、Notion設定の有無に関係なくサンプルデータで確認できます。
...
3. サイドバーの `Use sample data` がオフであることを確認する
4. `Refresh from Notion` を押す
5. 画面上部の `Debug / Status` を確認する

`Debug / Status` では以下を確認できます。

- 現在のデータソース: `sample` または `notion`
- `Use sample data` の現在値
